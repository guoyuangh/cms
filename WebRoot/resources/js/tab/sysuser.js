...
var selectCheckBoxName = "id";//checkbox id 的name值
...
function showAddModal(){
	var url = appCtx+"admin/sysuser/sysuserForm.do";
	$("#loading").modal("show");
	$.post(url,function(data){
		$("#loading").modal("hide");
		oneRowModalTitle.html("新增管理员");
...
function deleteModalDo(id){
	doConfirm("确认删除所选记录？",function(result){
		if(result){
	        var url = appCtx+"admin/sysuser/batchDelete.do";
	        var params = {};
	        params.ids = id;
	    	$("#loading").modal("show");
...
function saveOrUpdateForm(){
    ...
    if(flag){
        var url = appCtx+"admin/sysuser/insertOrUpdate.do";
